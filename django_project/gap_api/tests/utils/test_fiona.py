...
from gap_api.models.location import UploadFileType
...
class TestUtilsFiona(TestCase):
    ...
    def test_open_fiona_collection_shp(self):
        ...
        collection = open_fiona_collection(
            shape_file_path, UploadFileType.SHAPEFILE)
    ...
    def test_open_fiona_collection_gpkg(self):
        ...
        # test using filepath
        collection = open_fiona_collection(
            gpkg_file_path, UploadFileType.GEOPACKAGE)
        self.assertEqual(len(collection), 3)
        collection.close()
        ...
        with open(gpkg_file_path, 'rb') as file:
            mem_file = InMemoryUploadedFile(
                ...
                file_stats.st_size, None)
            collection = open_fiona_collection(
                mem_file, UploadFileType.GEOPACKAGE)
            self.assertEqual(len(collection), 3)
        collection.close()
        ...
        with open(gpkg_file_path, 'rb') as file:
            ...
            with open(tmp_file.temporary_file_path(), 'wb+') as wfile:
                wfile.write(file.read())
            collection = open_fiona_collection(
                tmp_file, UploadFileType.GEOPACKAGE)
            self.assertEqual(len(collection), 3)
        collection.close()
    ...
    def test_open_fiona_collection_geojson(self):
        ...
        # test using filepath
        collection = open_fiona_collection(
            geojson_file_path, UploadFileType.GEOJSON)
        ...
        with open(geojson_file_path, 'rb') as file:
            mem_file = InMemoryUploadedFile(
                ...
                file_stats.st_size, None)
            collection = open_fiona_collection(
                mem_file, UploadFileType.GEOJSON)
            self.assertEqual(len(collection), 1)
        collection.close()
        ...
        with open(geojson_file_path, 'rb') as file:
            ...
            with open(tmp_file.temporary_file_path(), 'wb+') as wfile:
                wfile.write(file.read())
            collection = open_fiona_collection(
                tmp_file, UploadFileType.GEOJSON)
            self.assertEqual(len(collection), 1)
        collection.close()
